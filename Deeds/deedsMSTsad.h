...
//	srand (time(0) );
	...
	float* im1; float* im1b;// pointer to pixels
	int M,N,O,K;
	char* header;
	...
	pthread_t thread1, thread2, thread1b, thread2b;
	int  iret1, iret2;
	...
	for(int level=0;level<maxlevel;level++){
	   ...
	   // mind1.im1=im1; mind1.mindq=im1_mind; mind1.qs=max(min(quant1,2.0f),1.0f); //qs determines size of patches for MIND
		...
		pthread_create( &thread1, NULL, regularisation, (void *) &reg1);
		pthread_create( &thread2, NULL, regularisation, (void *) &reg2);
		pthread_join( thread1, NULL);
		pthread_join( thread2, NULL);
