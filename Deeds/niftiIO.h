...
{		// fixedin,im1b,M,N,O,K,header
		// movingin,im1,M,N,O,K,header
	
	FILE * pFile;
	int i,j,offset;
	size_t result;
	
	pFile = fopen (str2, "rb" );  //只读打开一个二进制文件，只允许读数据
	if (pFile==NULL) {fputs ("File error",stderr); exit (1);}
	
	unsigned int X=0;
	fseek(pFile,0,0); //函数设置文件指针stream的位置...用法见声明即可懂
	fread(&X,4,1,pFile); //从stream中读数据到buffer，最多读取count个项，每个项size个字节，如果调用成功返回实际读取到的项个数（小于或等于count），如果不成功或读到文件末尾返回 0。
	
	//cout<<"Magic number "<<X<<"\n";
	int dims[5];
	short dim;
	for (i=0;i<5;i++){
		fseek(pFile,40+i*2,0);  //设置文件指针stream...偏离文件头40+i*2...结束到50.
		fread(&dim,2,1,pFile);  
		dims[i]=dim;
	}
	M=(int)dims[1];
	...
	float pixdims[8];
	fseek(pFile,76,0);
	fread(&pixdims,8*4,1,pFile);   // 8个pixel...4字节
